      ...
      Promise.all([load(src), backSrc ? load(backSrc) : Promise.resolve(null)])
        .then(([frontTexture, backTexture]) => {
          ...
          if (options.current.autoCrumple) {
            const dur = Math.max(0.2, finite(options.current.crumpleDuration, 0.55)) * 1000;
            const target = clamp(finite(options.current.crumpleAmount, 0.85), 0, 1);
            // Brief beat so the document reads before the grab.
            autoTimer = window.setTimeout(() => {
              if (disposed) return;
              start();
              // Phase 1 — gather loosely, with a slight twist.
              amount.target = target * 0.55;
              tiltX.target = 0.1;
              tiltY.target = -0.09;
              // Phase 2 — crush tight, counter-twist, then settle.
              autoTimer = window.setTimeout(() => {
                if (disposed) return;
                ...
                autoTimer = window.setTimeout(
                  ...
                  Math.max(320, dur * 0.55)
                );
              }, Math.max(380, dur * 0.7));
            }, 420);
          }
          wake();
        })
